...
4. **Footer badges around the spinning medallion** (bottom-center) — PREPPED, see `FOOTER-ASSETS.md`: LEFT = Buy-Me-a-Coffee "SUNO credits" badge; RIGHT = `footer/coffee-right.gif` (coffee cup, white keyed to transparent, 240px, animated). QR code = Chris doing later. Still need to BUILD the footer row and mount them.
5. **HOMEPAGE SWAP** — final, on Chris's "go".

## GROK COLLABORATION (new)
Chris set up a Claude↔Grok team-up. **Grok** (in Chris's browser, logged in) generates assets: it made the 13 animated glass-marble alpha GIFs and is fixing the app. **Claude** does all code + deploy. Grok delivers assets to Claude via public URL (curl-able) or by Chris saving files into `C:\Users\crafu\cfunkycreationsllcatx\marbles_grok\`. Watch for Grok to hand over the 13 marbles + the fixed app zip — mount/deploy them when they arrive.
